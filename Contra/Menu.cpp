...
{
    // Load background
    if (!m_backgroundTexture.loadFromFile("backgound_menu2.jpg"))
    {
    ...
    m_title.setPosition({ 720.f, 180.f });  // sửa cú pháp
    ...
    for (size_t i = 0; i < labels.size(); ++i) {
        ...
        text.setPosition({ 740.f, 300.f + static_cast<float>(i) * 60.f }); // sửa cú pháp
